---
title: G.711 A-law Encoder (Run)
html_meta:
    description: This article explains how you can use Transcoder to encode a WAV file to G.711 A-law compressed audio format.
taxonomy:
    category: docs
---
...
This article explains how you can use [Transcoder::run](https://doc.avblocks.com/core/latest/classprimo_1_1avblocks_1_1_transcoder.html#a31cbef423193a454b2634083cfb9b5cb) to encode a WAV file to G.711 A-law compressed audio format.

The code snippets in this article are from the [enc_g711_alaw_file](https://github.com/avblocks/avblocks-cpp/tree/main/samples/darwin/enc_g711_alaw_file) macOS sample.
...
This code takes a WAV file and encodes it to compressed G.711 A-law format.
...
The output socket defines where and how the encoded audio data will be written. In this case, we're configuring it to output a G.711 A-law WAV file. The socket represents the output destination, while the pin represents the specific audio stream within that destination. The AudioStreamInfo object specifies the audio format parameters - G.711 A-law typically uses 8000 Hz sample rate and mono channel, which are set explicitly in this example.
...
See the [build instructions](https://github.com/avblocks/avblocks-cpp/blob/main/docs/build-mac.md) for macOS and the [enc_g711_alaw_file](https://github.com/avblocks/avblocks-cpp/tree/main/samples/darwin/enc_g711_alaw_file) example for details.
...
The following example encodes input file `./assets/aud/express-dictate_8000_s16_1ch_pcm.wav` into output file `express-dictate_g711_alaw.wav`:
