---
title: Opus Decoder (Run)
html_meta:
    description: This article explains how you can use Transcoder to decode an Opus audio file to uncompressed PCM WAV.
taxonomy:
    category: docs
---
...
This article explains how you can use [Transcoder::run](https://doc.avblocks.com/core/latest/classprimo_1_1avblocks_1_1_transcoder.html#a31cbef423193a454b2634083cfb9b5cb) to decode an Opus audio file (in OGG container) to uncompressed PCM WAV.

The code snippets in this article are from the [dec_opus_file](https://github.com/avblocks/avblocks-cpp/tree/main/samples/darwin/dec_opus_file) macOS sample.

## Source Audio

For source we use the `Everybody-TBB.opus` file from the [AVBlocks Assets](https://github.com/avblocks/avblocks-assets/releases) repository. After downloading and unzipping you will find `Everybody-TBB.opus` in the `aud` subdirectory.

## Code

This code takes an Opus file and decodes it to uncompressed LPCM WAV file.
...
This is the main decoding function that ties everything together. First, we create an input socket that points to the Opus file we want to decode. Then we create the output socket using our helper function. The transcoder is the core component that performs the actual decoding - it takes the compressed Opus data from the input and converts it to uncompressed LPCM data for the output. 
...
See the [build instructions](https://github.com/avblocks/avblocks-cpp/blob/main/docs/build-mac.md) for macOS and the [dec_opus_file](https://github.com/avblocks/avblocks-cpp/tree/main/samples/darwin/dec_opus_file) example for details.
